...
---
*Last updated: 2026-08-25 — Phase 67 (kiosk tax quote-charge correctness) complete and deployed: the INV-000160 quote≠charge seam is closed — all silent 5% tax fallbacks removed (client + server), unresolvable tax fails closed naming the item (catalog builder now serves null, never a fabricated 0), and a live pre-charge assertion rejects any client/server total divergence before the terminal is charged (server discount rounding mirrors the client per-line, so legitimate discounted carts pass). Verified 15/15 must-haves; review findings (2 critical, 5 warning) fixed same-phase. New telemetry: `kiosk.total_mismatch`, `kiosk.recipe_total_mismatch` (recipe-path tax methodology deferred to a future phase). Phase 64 (linking & search correctness) code shipped same deploy train; its two human verifications remain open in 64-HUMAN-UAT.md. Off-theme aside — Phase 72 (Beer & Cider launch announcement pages) shipped 2026-08-25: `beer.html` + `cider.html` built on the about.html shell with placeholder business copy, site-wide Beer/Cider nav, homepage launch banners (stale "Beer Is Coming" waitlist retired), verified 13/13 + review CR-01/WR-01 fixed same-phase; committed to `main`/staging, owner fills placeholders and runs the prod push per `72-PROMOTE-STEPS.md` (not a v4.5 money-path requirement).*
