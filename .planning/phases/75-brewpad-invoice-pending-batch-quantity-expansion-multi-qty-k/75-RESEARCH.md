...
1. **What does "next reconcile" in D-01 concretely trigger for INV-000171 specifically?**
   ...
   - What's unclear: whether Phase 75 is expected to also modify the date-window scan's skip condition (broader risk, touches the "don't re-show already-fully-linked invoices" UX) so that a fully-automatic re-scan surfaces INV-000171, or whether a one-time manual re-import (single-invoice mode, `?number=INV-000171`, which has no skip filter) is an acceptable "auto-backfill on next reconcile" for this phase.
   - Recommendation: plan the `unit_total` fix as the core, always-in-scope deliverable. Treat "does the date-window scan need to become count-aware" as a separate, explicitly-scoped decision — default to NOT touching the date-window skip logic unless the owner confirms fully-hands-off backfill (not just a manual re-import) is required, since that skip condition intentionally prevents already-linked invoices from cluttering the Pull-from-Zoho candidate list for the common (correct) case.
...
2. **Should `createBatchesFromSale` and the bulk-create route's kit-expansion logic be unified into one shared helper as part of this phase, or left as parallel implementations with the bug ported over?**
   ...
   - What's unclear: whether refactoring into a shared helper is in-scope for a bug-fix phase (CLAUDE.md working principle: "don't over-engineer... implement the simplest solution for simple problems") or should be flagged as a follow-up.
   - Recommendation: the planner should decide based on phase-granularity config (`.planning/config.json` `granularity: "coarse"`) — a shared-helper refactor is a defensible small addition (low risk, same file, prevents recurrence) but is not strictly required to close the bug. Surface as a discretionary task, not a mandatory one.
