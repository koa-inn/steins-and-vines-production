...
- **D-01 (revised 2026-08-26 — see note below):** **Future-automatic + one-time manual backfill for the pre-fix invoice.** The fix makes the invoice→pending-batch derivation quantity-correct and idempotent, so every *future* multi-qty kit invoice yields N pending batches automatically on its first reconcile. The single pre-fix invoice **INV-000171** is corrected by a **one-time manual single-invoice re-import** (`GET /api/batch/scan-invoices?number=INV-000171` → Create Batches) after the prod middleware deploy. Idempotency remains essential — re-import and re-poll must NEVER duplicate (guaranteed by D-02's reconcile-against-existing rule + stable per-unit identity), and the regression suite must prove idempotency across repeated polls.
  - **Why revised:** The original D-01 required INV-000171 to self-correct 1→3 "with no manual step." Research (75-RESEARCH.md) confirmed no automatic re-scan resurfaces an already-linked invoice — the date-window "Pull from Zoho" scan intentionally *skips* any invoice already carrying a `cf_batch_status` (`pos.js:3156-3160`). Delivering literal auto-backfill would require making that skip filter count-aware, which is higher-risk and overlaps the Phase 62/63 reconciliation model. Owner decided 2026-08-26 to relax D-01: ship the scoped idempotent fix + one-time manual re-import for the single legacy invoice, and explicitly **do not** modify the date-window skip filter in this phase.
