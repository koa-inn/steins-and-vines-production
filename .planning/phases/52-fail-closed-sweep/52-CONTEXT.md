...
> **Wave-reshape note (planner, 2026-07-03 — intentional, not a contradiction of the "helper → M1–M5 → auth/validation" sequence above).** The final plan set splits into 5 file-disjoint plans and reshapes the wave order per this section's explicit "Planner may re-shape; keep waves parallel-safe by file" grant. The *only* hard sequencing constraint is that M1/M4/M5 consume the shared helper, so **52-02 (M1/M4/M5) is wave 2 and `depends_on` 52-01 (the helper, wave 1)**. Everything else has NO functional dependency on the helper or on each other, so the auth/validation/disposition plans run in wave 1 concurrently with the helper: **52-03 (pos.js M2+M3), 52-04 (items.js M20 + taxes.js M6), 52-05 (catalog.js M7 + recipes/gift-cards M8)**. This is safe because every wave-1 plan owns a disjoint set of files (redis-guard.js / pos.js / items.js+taxes.js / catalog.js+recipes.js+gift-cards.js — no overlap), and pos.js's two findings (M2, M3) are co-located in a single plan (52-03) so there is no pos.js write conflict. The original prose sequence ("helper → M1–M5 → auth/validation") describes *logical dependency*, which is preserved; it does not mandate that the independent auth/validation fixes wait for M1–M5 to finish.
