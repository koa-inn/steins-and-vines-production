...
> Both questions below were planning decisions, not blocking unknowns, and are resolved in the Phase 76 plans: Q1 → deferred to the executor in Plan 76-03 Task 3 (simplify `initGoogleAuth`/`doSilentRefreshOnLoad` and record the choice in 76-03-SUMMARY.md); Q2 → resolved as the recommended single global interceptor, implemented in Plan 76-03 Task 2 (a standalone exported `_handleMiddlewareResponse` keyed on the middleware `res.status === 401`).

1. **[RESOLVED — Plan 76-03 Task 3]** **Should `doSilentRefreshOnLoad`'s login-time fallback survive the migration?**
...
2. **[RESOLVED — Plan 76-03 Task 2]** **Exact final shape of the "middleware 401 → logout" trigger after `adminApiGet`/`adminApiPost` no longer live-check `isUnauthorizedError` per-call.**
