...
**Cookie consent built 2026-09-17:** `js/consent.js` gates Google Tag Manager (and with it GA4, Google Ads, the Meta pixel and Metricool) behind an unobtrusive Decline/Accept notice, remembered for a year, with a footer "Cookie settings" control; Privacy §4 wording restored. Details in `docs/TRACKING.md`. **reCAPTCHA notice and waitlist consent built 2026-09-17:** the checkout carries the Google reCAPTCHA notice with both policy links, and the beer waitlist form shows a CASL consent statement (sender, purpose, unsubscribe, privacy link) that travels with the signup and is logged by the middleware as the consent record. The policy build list is complete. **Still with the owner:** the lawyer review (seven questions below), the Zoho organisation legal name, and the accountant check on the kit zero-rating.
